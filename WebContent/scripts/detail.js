(function() {
  ...
  function ajax(method, url, data, successCallback, errorCallback) {
    var xhr = new XMLHttpRequest();//讓我們使用ajax機制

    xhr.open(method, url, true);
    //onload onerror:對事件的監聽
    xhr.onload = function() {//onload:成功時調用; 是掛在 XMLHttpRequestEventTarget(XMLHttpRequest 的子類)之下，可以直接xmlhttprequest調用
      if (xhr.status === 200) {
    ...
    if (data === null) {
      xhr.send();
    } else {//set content text to json mode
