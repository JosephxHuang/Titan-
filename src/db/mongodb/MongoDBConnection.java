...
public class MongoDBConnection implements DBConnection {
	...
	public MongoDBConnection() {
		// Connects to local mongodb server.//Create a connection
	...
	public Set<String> getCategories(String itemId) {
		...
		if (iterable.first() != null && iterable.first().containsKey("categories")) {
			@SuppressWarnings("unchecked")//warning for強制轉化
	...
	public List<Item> searchItems(double lat, double lon, String term) {
		TicketMasterAPI tmAPI = new TicketMasterAPI();//same as from mysql because it is not related to db but API
	...
	public void saveItem(Item item) {
		if (db == null) {//insert method
			return;
		}
		FindIterable<Document> iterable = db.getCollection("items").find(eq("item_id", item.getItemId()));//"find" is like the IGNORE key word in mysql to prevent duplication
		if (iterable.first() == null) {//do not find duplicated item, so we can continue
